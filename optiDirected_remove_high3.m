...
for realization=1:30%30%1:10%10:-1:6%1:10 % the number of numerical replicates
tic;
...
r2_original = real(eigv(end))/real(eigv(2));

r_original = real(eigv(end))/real(eigv(2));%SpreadEigenvalues;%eigv(end)/eigv(2);
% target value of the eigen-ratio:  too small makes convergence harder
r_targetFinal = 1;%0.1;%1;%
...
for i = 1:Run
    ...
    while r_new > r_target || Metropolis==0
        ...
        InitializeDeletedLinks=1;
        while InitializeDeletedLinks==1
            aij = adj;
        idx = find(adj == 1);
        ...
        if length(unique(aaa))==NumberNode %% some node becomes isolated
            break;
        end
        end
        %%%%%%%
       ...
       lap=Laplacian2(aij);
        ...
        d=length(find(aij == 1))/NumberNode/(NumberNode-1);
        SpreadEigenvalues=1/d^2/(NumberNode-1)/(NumberNode-2)*sum(abs(eigv(2:end)-lambdabar).^2);
        r2_optimal = real(eigv(end))/real(eigv(2));%eigv(end)/eigv(2);
        
        r_new =real(eigv(end))/real(eigv(2));%SpreadEigenvalues;
...
Summary.EigenRatio2(NumberNode,1)=r2_original;
Summary.EigenRatio2(NumberNode,2)=r2_optimal;
