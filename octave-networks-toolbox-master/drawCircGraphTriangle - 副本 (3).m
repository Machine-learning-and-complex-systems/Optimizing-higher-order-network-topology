...
for k=1:n
  ...
  text(1.15*x(k),1.15*y(k),strcat(num2str(k)),"fontsize",fontsize);
  plot1.Color(4) = alpha;
  hold off; hold on;
end
...
for e=1:length(edges)
    ...
    %plot((x(ii)+x(jj))/2,(y(ii)+y(jj))/2,'->','Color',cc(kk,:), 'LineWidth',1);
    plot_arrow( x(ii),y(ii),(x(ii)+x(jj))/2,(y(ii)+y(jj))/2,'linewidth',1,'color',cc(kk,:),'facecolor',cc(kk,:), 'edgecolor',cc(kk,:),'headwidth',0.05,'headheight',0.03)
    nodeX(e)=x(ii);
    nodeY(e)=y(ii);
